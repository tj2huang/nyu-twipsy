...
def fpl_summary(in_dir, out_dir, file_name):
    try:
        ...
        df_alc = df.alc.groupby([df.index.day, df.index.hour]).agg({'reflecting': sum, 'total': len})
        df_fpa = df.fpa.groupby([df.index.day, df.index.hour]).agg({'reflecting': sum, 'total': len})

        df_casual.to_csv('casual_' + file_name)
        df_looking.to_csv('looking_' + file_name)
        df_reflecting.to_csv('reflecting_' + file_name)
        df_alc.to_csv('alc_' + file_name)
        df_fpa.to_csv('fpa_' + file_name)
...
def main(argv):
    ...
    # parallel
    p = Pool(cores)
    args = [(out_dir, in_folder + '/' + f, 'tweets_' + str(i) + '.csv') for i, f in ennumerate(os.listdir(in_folder))]
    p.map(predict, args)
    # for args in dirs:
    #     predict(args)
